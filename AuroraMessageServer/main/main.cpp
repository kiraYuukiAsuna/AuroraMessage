...
int main(int argc, char* argv[]) {
	...
	while (true)  //客户端连接接受
	{
		...
		if (clientResource != nullptr) {
			...
			inet_ntop(AF_INET, &clientResource->clientAddress.sin_addr, ptr,
				INET_ADDRSTRLEN);
			std::cout << "port:" << clientResource->clientAddress.sin_port
				<< std::endl;
			std::cout << "clientAddress:" << ptr << std::endl;
			std::cout << "sendBuffer:" << clientResource->sendBuffer << std::endl;
			std::cout << "recieveBuffer:" << clientResource->recieveBuffer
				<< std::endl;
